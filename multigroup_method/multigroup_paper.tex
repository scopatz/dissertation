...
solve fundamental physical systems, these codes parameterize the system.   Interpolations
...
reactors model-able from fast reactors (FR) and light-water reactors (LWR), the flux spectrum evolution and 
...
is independent of the flux.  Parameterizing based on fluence was successful in R1G because 
...
displays the group structure implemented.  This group structure is designed to capture resonance 
...
as well as the 17x17 lattice that were used.  Numeric values for the lattice may be inferred from 
those of the unit cell.  Reflecting boundary conditions were applied to the edges of the lattice.
...
fuel mass fractions for an initial heavy metal (IHM) with a 4.5\% \nuc{U}{235} enrichment.
...
Note that these mass fractions may be perturbed from the values in these tables.  Here however 
this is only done for the fuel region.
...
live long enough that they have noticeable impacts on the fuel cycle (specifically with regards to 
...
The term $4 \pi b^2$ represents a base estimate of the scattering cross section
(the so-called bound scattering cross section $\sigma_b$).  
%Moreover, the $\left(1 - \frac{2E}{931.46 \cdot m_n}\right)$ term in equation \ref{scat_ce} is 
%a relativistic correction factor for $b$.  
...
data not available), a zero value is finally assumed for this group constant.  A zero value, rather 
...
RMG is partially given by the specific power, $p_s$ [MW/kg].  Denote the one-group macroscopic 
...
probability accounts for the fact that the reactor is modeled as an infinitely reflected system, whereas
...
vary by reactor type.  The disadvantage factor $\zeta_{tg}$ is calculated from two-region lattice functions
...
The `total' values $\zeta_t$ show good agreement between the RMG and the traditional 
thermal disadvantage factor.  Depending on geometry, $\zeta_t$ for an LWR lies approximately 
