...
dominant physics methodology.  In seeking to expand the types of reactors model-able from 
fast reactors and light-water reactors, the flux spectrum must be folded back into the 
...
\end{equation}
Here, $\zeta_{tg}$ is the thermal disadvantage factor per group.  This may computed via the macroscopic 
...
probability accounts for the fact that the reactor is modeled as an inifintely relefected system, whereas
...
to work by variying $P_{NL}$ and $\zeta_{tg}$.  Reasonable limits on which to adjust these two parameters 
...
within appropriate limits for all groups.  Moreover, differences in Figures \ref{spec_BOL} \& 
\ref{spec_EOL} is partially explained by the difference in regions for which the flux is defined.
The Serpent curves here represent a homogenized full-core, while the RMG spectra are only for the 
fuel region.
...
to ensure that no reaction is far off from a benchmark value.  Tables 
