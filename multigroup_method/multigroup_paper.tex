...
cross section for a free gas \cite{Yamamoto2006, Mattes2005}.
\begin{equation}
\label{scat_ce}
...
\begin{equation}
\label{scat_kern}
S(\alpha, \beta) = \frac{1}{\sqrt{4\pi\alpha}} \mbox{Exp}(-\frac{\alpha^2 + \beta^2}{4\alpha})
\end{equation}
...
Because neutron transport is not performed here, the double differential cross section
may be integrated over all solid angles.
\begin{equation}
\label{scat_de}
\frac{d\sigma_s(E\to E^\prime)}{dE^\prime} = \frac{b^2 M_A}{2Em_n} \cdot 
            \left(1 - \frac{2E}{931.46 \cdot m_n}\right) \cdot \left[Q^-(\alpha, \beta) - Q^+(\alpha, \beta)\right]
\end{equation}
where 
\begin{equation}
\label{scat_q}
...
\end{equation}
This is then used to generate group constants for all reactions from perturbation information
...
\end{equation}
Here, $\zeta_{tg}$ is the disadvantage factor per group.  This may computed via the macroscopic 
...
to the reactor model.  Note that together the $P_{NL}$ and $\zeta_{tg}$ are functionally adjustment parameters
that which may be used to account for fundemental modeling errors in the geometery.  The non-leakeage 
probability accounts for the fact that the reactor is modeled as an infinitely relefected system, whereas
any real system will have physical limits.  By analogy, the thermal disadavantage factor corrects the flux 
for errors stemming from a two-region lattice diffusion theory approximation.  Thus, the RMG may be `tuned'
to work by variying $P_{NL}$ and $\zeta_{tg}$.  Reasonable limits on which to adjust the non-leakage probability
vary by reactor type.  The thermal disadvantge factor is calculated from two-region lattice functions.
