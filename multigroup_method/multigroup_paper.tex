...
$i$, times are indexed by $t$, pertubations are indexed by $p$, and energy is indexed by $g$ with 
...
but can not be generated by the reactions in these libraries is a perrenial issue.
...
The final psuedo-tally that is calculated via serpent is the fission neutron energy spectrum, $\chi(E)$.
The continious energy cross section data libraries do not contian information on $\chi(E)$.  However, 
...
may have signifigcant fuel cycle importance and yet do not have the highest-fidelity data available.
...
small quantities.  If $n$ indexes $N=64$ groups from Cinder and $E$ [MeV] denotes the enegry boundaries, 
...
The absorption cross section $\sigma_{a,ipg}$ is simply the sum of its constituient elements, as 
...
represeentation of the scattering kernel $S(\alpha, \beta)$ for a free gas.
...
available and which do not have enough of a fuel cycle impact that warrents computing group constants 
...
for almost 3000 nuclides at a variety of energies \cite{KAER2000}.  Specifially, data for
thermal (2.53E-08 [MeV]) and fission spectrum average (taken as 1.0 [MeV]) were used as represetative
...
\end{equation}
Lastly, it is required that the constituent absorpotion tallies sum 
...
However, the advantage of the RMG method here is that the values of the reactor paramaters
